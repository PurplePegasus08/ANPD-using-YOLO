...
elif not os.path.exists(image_dir) or not os.path.exists(result_dir):
    os.makedirs(image_dir, exist_ok=True)
    os.makedirs(result_dir, exist_ok=True)
    folders_created = True

# ==== First run: only make folders ====
if folders_created or len(sys.argv) < 3:
...
# ==== Load models ====
model = YOLO("best3.pt")  # Adjust path if needed
